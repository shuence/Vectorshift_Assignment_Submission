...
scope = os.getenv('AIRTABLE_SCOPE')
CACHE_EXPIRY = int(os.getenv('CACHE_EXPIRY', 600))

# Setup URLs and encoded credentials
authorization_url = f'https://airtable.com/oauth2/v1/authorize?client_id={CLIENT_ID}&response_type=code&owner=user&redirect_uri={requests.utils.quote(REDIRECT_URI)}'
...
async def authorize_airtable(user_id, org_id):
    ...
    await asyncio.gather(
        add_key_value_redis(f'airtable_state:{org_id}:{user_id}', json.dumps(state_data), expire=CACHE_EXPIRY),
        add_key_value_redis(f'airtable_verifier:{org_id}:{user_id}', code_verifier, expire=CACHE_EXPIRY),
    )
...
def fetch_items(
    access_token, url, aggregated_response, offset=None
):
