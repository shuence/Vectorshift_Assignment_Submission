...
# Setup URLs and encoded credentials
encoded_client_id_secret = base64.b64encode(f'{CLIENT_ID}:{CLIENT_SECRET}'.encode()).decode()
authorization_url = f'https://api.notion.com/v1/oauth/authorize?client_id={CLIENT_ID}&response_type=code&owner=user&redirect_uri={requests.utils.quote(REDIRECT_URI)}'
...
async def authorize_notion(user_id, org_id):
    ...
    # URL safe encode the state for security - similar to how it's done in hubspot.py
    encoded_state = base64.urlsafe_b64encode(json.dumps(state_data).encode('utf-8')).decode('utf-8')
    
    # Store the original state data in Redis for verification
    await add_key_value_redis(f'notion_state:{org_id}:{user_id}', json.dumps(state_data), expire=CACHE_EXPIRY)
    
    # Return the authorization URL with the encoded state
...
async def oauth2callback_notion(request: Request):
    ...
    # Decode the state using the same method used for encoding
    try:
    ...
    await add_key_value_redis(f'notion_credentials:{org_id}:{user_id}', json.dumps(response.json()), expire=CACHE_EXPIRY)
